...
export const dataManager = {
  init: () => {
    if (!localStorage.getItem(KEYS.POSTS)) {
        // Posts seed would go here or stay in constants
    }
    
    // Always ensure resources exist
    ...
    if (!localStorage.getItem(KEYS.STUDENTS)) {
        const initialStudents = [
            { id: 'std-1', name: 'أمين التلميذ', username: 'amin', password: '123', grade: '2 باكالوريا', status: 'active', joinDate: '2023-09-01', stats: { studyHours: 12, commitmentRate: 85, weeklyProgress: [40, 60, 55, 80, 70, 85, 50] } }
        ];
